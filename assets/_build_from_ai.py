...
烤成符合 assets.js 清单规范的图集：tileset.png（6x2，cell=64）与 david.png（6x4，cell=64）。
...
def place_bottom_center(sprite, cell=CELL, pad=2, max_frac=1.0, scale=None):
    ...
    scale=None 时按自身包围盒自适应；给定 scale 时用统一比例（避免逐帧抖动，顶部允许裁切）。"""
...
# ---------- 角色 david.png（6 帧 × 4 朝向） ----------

def _blobs(im, athresh=24, min_area=120):
    ...
    for y in range(H):
        for x in range(W):
            if mask[y, x] and not seen[y, x]:
                ...
                while q:
                    cy, cx = q.popleft(); n += 1; xs += cx; ys += cy
                    minx = min(minx, cx); maxx = max(maxx, cx)
                    miny = min(miny, cy); maxy = max(maxy, cy)
                    for dy, dx in ((1, 0), (-1, 0), (0, 1), (0, -1)):
                        ny, nx = cy + dy, cx + dx
...
def build_david(cols=6, rows=4):
    src = Image.open(os.path.join(SRC, "david_walk_6x4.png"))
    keyed = keyout_white(src)
    W, H = keyed.size
    blobs = _blobs(keyed)
    # 每个连通域按质心归到最近的 (行, 列) 槽位；同槽位的碎块（杖/石子）合并成整体包围盒
    grid = {}
    for cx, cy, minx, miny, maxx, maxy in blobs:
    ...
    # 统一缩放：用各帧高度中位数定一个比例，保证行走时人物大小恒定（杖尖偶尔顶部轻微裁切可接受）
    heights = sorted(bb[3] - bb[1] for bb in grid.values())
    med_h = heights[len(heights) // 2]
    uni_scale = (CELL * 0.96) / med_h
    ...
    out.save(os.path.join(HERE, "david.png"))
    print("david.png", out.size, "figures", len(grid))
...
def build_tileset():
    ...
    # 高草丛：草地 + 灌丛
    tgrass = grass.copy(); tgrass.alpha_composite(place_bottom_center(crop(16), pad=4, max_frac=0.8))
    # 花丛：草地 + 花
    flower = grass.copy(); flower.alpha_composite(place_bottom_center(crop(14), pad=4, max_frac=0.8))
    # 洞穴：石板 + 暗色洞口
    cave = cobble.copy()
    ...
    tiles = {
        (0, 0): grass, (1, 0): tgrass, (2, 0): flower,
        (3, 0): path,  (4, 0): drygrass, (5, 0): cobble,        # FOLD=干草, HILL=石板
        (0, 1): on_grass(10), (1, 1): on_grass(12), (2, 1): on_grass(7),
        (3, 1): on_grass(8),  (4, 1): on_grass(9),  (5, 1): cave,
    }

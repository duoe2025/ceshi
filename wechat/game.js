...
(() => {
  ...
  var CanvasPainter = class {
    ...
    fillRect(sx2, sy2, w, h, color) {
      if (w <= 0 || h <= 0)
        return;
      this.ctx.fillStyle = cssColor(color);
      this.ctx.fillRect(sx2, sy2, w, h);
    }
    fillCircle(sx2, sy2, r, color) {
      if (r <= 0)
        return;
      this.ctx.fillStyle = cssColor(color);
      this.ctx.beginPath();
      this.ctx.arc(sx2, sy2, r, 0, Math.PI * 2);
      this.ctx.fill();
    }
    fillEllipse(sx2, sy2, rx, ry, color) {
      if (rx <= 0 || ry <= 0)
        return;
      this.ctx.fillStyle = cssColor(color);
      this.ctx.beginPath();
      this.ctx.ellipse(sx2, sy2, rx, ry, 0, 0, Math.PI * 2);
      this.ctx.fill();
    }
    fillPoly(pts, color) {
      if (pts.length < 3)
    ...
    strokeCircle(sx2, sy2, r, lineWidth, color) {
      if (r <= 0)
        return;
    ...
    strokeArc(sx2, sy2, r, a0, a1, lineWidth, color) {
      if (r <= 0)
        return;
      this.ctx.lineWidth = lineWidth;
      this.ctx.strokeStyle = cssColor(color);
      this.ctx.beginPath();
      this.ctx.arc(sx2, sy2, r, a0, a1);
      this.ctx.stroke();
    }
  };
...
` + (this.touchUI ? "\u70B9\u51FB\u5C4F\u5E55\u91CD\u65B0\u5F00\u59CB" : "\u6309 R \u91CD\u65B0\u5F00\u59CB");
    ...
    drawEnemyEntity(e, camX, camY) {
      ...
      if (e.sprite === "spirit") {
        ...
        spirit(p, sx2 - 6, sy2 - 10 + bob, e.scale, e.hurt > 0);
      } else {
        lion(p, sx2 - 8, sy2 - 8, e.scale, e.hurt > 0);
      }
      const col = e.type === "spirit" ? rgba(154, 127, 208) : e.ambient ? rgba(200, 150, 90) : rgba(217, 83, 79);
      this.nameTag(`${e.name} Lv.${e.level}`, sx2 + 16, sy2 - 14, col);
      this.enemyHpBar(sx2 + 16, sy2 - 28, e);
      if (e.hurt > 0)
        e.hurt--;
    }
    ...
    drawEquipPanel() {
      ...
      slots.forEach((s) => {
        ...
        this.text(`${G.slotLabel(s)}\uFF1A`, x + 16, sy2, 14, rgba(207, 198, 184));
        if (it)
          this.text(it.name, x + 110, sy2, 14, G.itemColor(it));
        else
          this.text("\u2014", x + 110, sy2, 14, rgba(120, 120, 130));
        sy2 += 24;
      });
      const a = G.player.attr;
      const d = G.derived;
...
${lines.join("\n")}`, x + 16, y + 14, 15, rgba(244, 236, 216), w - 32);
  ...
  // assets/scripts/canvas/main.wechat.ts
